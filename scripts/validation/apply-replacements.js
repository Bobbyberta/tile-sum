...
/**
 * Script to apply word replacements to puzzle-data.js
 * Reads replacements.json and updates puzzle-data.js with the new words
 */
...
async function main() {
    try {
        console.log('Applying word replacements to puzzle-data.js...\n');
        ...
        for (const [puzzleNum, items] of sortedPuzzles) {
            console.log(`\nPuzzle ${puzzleNum}:`);
            for (const { oldWord, newWord, position } of items) {
                console.log(`  Position ${position}: "${oldWord}" → "${newWord}"`);
            }
        }
        ...
        console.log('3. Run: npm run validate:anagrams (to verify no anagrams remain)');
